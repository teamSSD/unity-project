...
# Resources.Load 호출 (antipatterns에 중복이지만 게이트 일관성을 위해 한 곳에)
N_RES_LOAD=$(xargs0 "$RUNTIME_LIST" grep -hE 'Resources\.Load' 2>/dev/null | wc -l | tr -d ' ')

# Assets/Resources/ 폴더 파일 수 (.meta 제외)
RES_DIR="$ROOT/Assets/Resources"
if [ -d "$RES_DIR" ]; then
    N_RES_FILES=$(find "$RES_DIR" -type f -not -name "*.meta" 2>/dev/null | wc -l | tr -d ' ')
else
    N_RES_FILES=0
fi
...
echo "asset_loading: Resources.Load=$N_RES_LOAD, Resources/files=$N_RES_FILES, AssetReference=$N_ASSETREF" >&2
