...
public class FarmTileLayoutContractTest
{
    ...
    public void CropLabel_IsBelowCrop_AndGaugeUsesCompactSize()
    {
        ...
        Assert.That(int.Parse(nameCanvas.Groups[1].Value), Is.LessThanOrEqualTo(-170),
            "작물명은 작물 스프라이트 아래에 배치되어야 합니다.");
        Assert.That(int.Parse(nameCanvas.Groups[2].Value), Is.LessThanOrEqualTo(80),
            "작물명 영역은 작물과 겹치지 않도록 컴팩트해야 합니다.");
