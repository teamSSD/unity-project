...
{
    /// <summary>실 유저 최적 플레이 근사: FarmAware 메뉴 선택 + AdaptiveSide 사이드 latch + Baseline 안전 로직.
    /// - 메뉴: 팜 crop overlap 우선, 마진 tiebreak
    /// - 사이드: 자산 100k 도달 후 영구 latch, 재고 있으면 서빙에 붙임
    /// - Upgrade: 초반 3일 유예 + 최근 3일 매출 healthy 시만
    /// - Purchase: side latch 상태 반영해 사이드 재료도 매수 대상 포함
    /// </summary>
    public class AdaptiveFarmSidePolicy : IPlayerPolicy
    {
        public string Name => "AdaptiveFarmSide";

        public int SideAdoptionThreshold = 100_000;
